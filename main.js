function doPost(e) {
  try {
    ...
    if (!e || !e.postData || !e.postData.contents) {
      return generarRespuesta("Error", "No se recibió carga útil en la petición", 400, { procesado: false });
    }
    ...
    if (camposFaltantes.length > 0) {
      return generarRespuesta("Error", "Faltan campos obligatorios: " + camposFaltantes.join(", "), 400, {
        procesado: false,
        camposFaltantes
      });
    }

    // 2. Seguridad: Sanitización de entrada y prevención de inyección CSV
    ...
    if (!sheetId) {
      return generarRespuesta("Error", "Fallo de configuración interna del servidor (SHEET_ID)", 500, { procesado: false });
    }
    const sheet = SpreadsheetApp.openById(sheetId).getSheets()[0];
    
    if (existeSolicitud(sheet, datosSanitizados.solicitudId)) {
      return generarRespuesta("Error", "Solicitud duplicada (ID: " + datosSanitizados.solicitudId + ")", 409, {
        procesado: false
      });
    }
    ...
    try {
      ...
      return generarRespuesta("Error", "Fallo el registro en Google Sheets", 500, { procesado: false });
    }
    ...
    if (discordExito) {
      return generarRespuesta("Exito", "Solicitud registrada y notificada correctamente", 200, { procesado: true });
    } else {
      // falla del Webhook -> persistencia exitosa pero notificación fallida
      return generarRespuesta("Exito Parcial", "Guardado en Sheets, pero fallo la notificación a Discord", 200, { procesado: true, warning: "Webhook fallido" });
    }

  } catch (error) {
    // JSON inválido -> code 400
    return generarRespuesta("Error", "JSON invalido o malformado", 400, { procesado: false });
  }
}
...
function enviarNotificacion(data) {
  try {
    ...
    if (!webhookUrl) throw new Error("Webhook URL no configurado en PropertiesService");
    ...
    let maskedEmail = emailParts.length === 2 ? 
    ...
    return resp.getResponseCode() < 300 ? { ok: true } : { ok: false, error: resp.getResponseCode() };
  } catch (e) { 
...
function generarRespuesta(status, message, code, extra) {
  const res = Object.assign({ status, message, code, timestamp: new Date().toISOString() }, extra || {});
